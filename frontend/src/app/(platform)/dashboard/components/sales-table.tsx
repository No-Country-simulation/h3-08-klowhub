...
function EarningsSalesTable({ data }: { data: EarningsSale[] }) {
  const columns: Array<ColumnDef<EarningsSale>> = [
    // Customer Avatar
    {
      accessorKey: 'customer.image_url',
      header: '',
    ...
    // tipo
    {
      ...
      header: 'Tipo',
      cell: (cell: any) => (
        <span className='gen-sale-type-success'>{cell.row.original.type.toString()}</span>
      )
    },
    // estado
    {
    ...
    // Ver Detalle
    {
      ...
      header: '',
      cell: (cell: any) =>
        <Link href={cell.row.original.details_url}>
...
export { SalesTable as DashboardSalesTable, EarningsSalesTable }
